...
class Trainer:
    ...
    def set_mixed_precision_context(self, mixed_precision_dtype):
        ...
        if mixed_precision_dtype is None:
            ...
            device_type = "cuda" if "cuda" in f"cuda:{self.gpu_id}" else "cpu"
            # TODO Otherwise, use 'torch.amp.autocast' context with the specified dtype.
            self.ctx = torch.amp.autocast(device_type=device_type, dtype=mixed_precision_dtype)
            # initialize GradScaler if mixed_precision_dtype is float16.
    ...
    def prepare_dataloader(self, train_dataset, eval_dataset):
        ...
        # TODO: Prepare the training DataLoader. Initialize 'DataLoadDataCollatorForSeq2Seqer' with 'train_dataset'
...
def load_pretrained_model(local_rank, model_path: str = ""):
    ...
    model = LoraModelForCasualLM(model = model,peft_config = lora_config)
